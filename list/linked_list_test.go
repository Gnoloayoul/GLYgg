...
func TestLinkedList_Add(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, tc.wantLinkedList.ChangeSlice(), tc.list.ChangeSlice())
		})
	}
}
...
func TestNewLinkedListOf(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		assert.Equal(t, tc.wantedSlice, list.ChangeSlice())
	}
}

func TestLinkedList_ChangeSlice(t *testing.T) {
	vals := []int{1, 2, 3}
